...
void passWidth(binTree b){
    if (b == NULL){
          cout << "Ошибка! Дерево пусто!"; exit(1);
       }
    int n = 0; int i = -1;
    char res[80];
    queue <binTree> q;
...
binTree enterBT (istream& stream){	   
	...
	if (ch=='/' || ch=='#') 
	   ...
	   return ConsBT(ch, p, q);
	    ...
	    cout << "Ошибка! Неверный символ: " << ch <<"\n";
	    exit(1);
	}
}
...
void readRoot(istream &stream, binTree& obj, char prev){	
  ...
  if (x == ')'){
      ...
      cout << "\nОшибка! Нельзя вводить пустое дерево."; 
      exit(1);
  }
...
do{
    ...
    while (str!=file && str!=con)
     ...
     cout << "Считать в формате КЛП - \033[1;34m\'1\' , \033[0m или в формате полной скобочной записи - \033[1;34m\'2\': \033[0m";
     cin >> st; 
    while (st!="1" && st!="2")
      {
        cout << "\033[1;31mОшибка ввода, попробуйте снова: \033[0m" ;
     ...
     if (str == file) {
         ...
         while(!in) {
          ...
          in.close();
       	  ...
       	  int flag = st[0] - '0';     
          if (flag == 1){ 
              cout << "\033[0;32mВведите узлы БД в формате КЛП (пустой узел - \'/\') \n:\033[0m ";
              res = enterBT(cin);
          }
          else {  
